...
import music from '/public/music/naruto-reg.mp3'; // Ensure you have a valid audio file path

// Make sure to import your styles.css in your main App.js or index.js
// so the '@font-face' rule is loaded globally.
...
// URL for the background music. Replace with your own if you have one.
const narutoMusicUrl = "/public/music/naruto-reg.mp3"; // Ensure this path is correct
...
const MusicPlayer = ({ audioUrl }) => {
    ...
    useEffect(() => {
        ...
        if (isPlaying) {
            // Attempt to play, but catch errors if browser blocks autoplay
            audioEl.play().catch(error => {
                console.warn("Autoplay was prevented by the browser. User must interact to play music.");
                setIsPlaying(false); // Set back to false if autoplay fails
            });
        } else {
            audioEl.pause();
        }
    ...
    // Cleanup to pause music when the component unmounts
    useEffect(() => {
        const audioEl = audioRef.current;
        return () => {
            if(audioEl) {
    ...
    return (
        <div className="absolute top-4 right-4 z-20">
            {/* The `autoPlay` attribute is a hint, but JS control is more reliable */}
...
function Form() {
    ...
    // Consistent styles for all form elements
